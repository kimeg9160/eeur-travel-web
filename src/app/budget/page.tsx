  ...
  if (!items.length) {
    return (
      <div className="text-center py-20">
        <h1 className="text-2xl font-bold text-slate-800 mb-4">💰 예산</h1>
        <p className="text-slate-500 mb-4">예산 데이터가 아직 없습니다.</p>
        <button onClick={seedBudget} className="bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700">
          📥 기본 예산 항목 생성
        </button>
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: "총 예산", value: `₩${totalKrw.toLocaleString()}` },
          { label: "확정 비용", value: `₩${fixedKrw.toLocaleString()}` },
          { label: "미확정", value: `₩${(totalKrw - fixedKrw).toLocaleString()}` },
          { label: "1인 일평균", value: `₩${Math.round(totalKrw / 2 / (trip.total_days || 12)).toLocaleString()}` },
        ].map((s) => (
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3">카테고리별 비율</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={110} dataKey="value" label={({ name, percent }: { name?: string; percent?: number }) => `${name || ""} ${((percent || 0) * 100).toFixed(0)}%`}>
                {pieData.map((_, i) => (
        ...
        {/* Progress bars */}
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3">확정 진행률</h3>
              ...
              return (
                <div key={g.category}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-slate-500">₩{g.fixed.toLocaleString()} / ₩{g.total.toLocaleString()}</span>
                  </div>
      ...
      <p className="text-xs text-slate-400 mt-4">💱 환율: 1 EUR = ₩{Number(trip.exchange_rate).toLocaleString()}</p>
    </div>
  );
}
